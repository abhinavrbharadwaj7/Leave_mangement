...
import DeliveryAnimation from './DeliveryAnimation'; // Import the 3D animation component
...
const EmployeeDashboard = () => {
  ...
  const handleLeaveSubmit = async (values) => {
    try {
      ...
      if (response.data.success) {
        ...
        fetchLeaveBalance(); // Refresh balance in background
        // Other refresh calls will happen after animation completes
      }
    } catch (error) {
  ...
  return (
    <>
      {showAnimation && <DeliveryAnimation onComplete={handleAnimationComplete} />}
            ...
            <Modal title="Request Time Off" open={modalVisible} onCancel={handleCancel} footer={null}>
              {showAnimation ? (
                <div style={{ height: 400, position: 'relative' }}>
                  <DeliveryAnimation onComplete={handleAnimationComplete} />
                </div>
              ) : (
